...
export default function Hero() {
  return (
    <section className="relative bg-[#1f1a29] overflow-hidden min-h-[85vh] lg:min-h-[600px] flex items-center">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 lg:py-16 grid lg:grid-cols-[1.1fr_0.9fr] gap-10 lg:gap-4 items-center relative z-10 w-full">
        {/* Left Content */}
        <div className="text-white space-y-4 animate-in slide-in-from-left duration-700 relative z-20">

          <div className="flex items-center gap-3">
            <h3 className="font-pacifico text-orange-500 text-3xl md:text-[2.2rem] tracking-wide relative">
              <span className="absolute -left-12 top-1 opacity-80 animate-spin" style={{ animationDuration: '10s' }}>🍅</span>
              Satisfy Your Cravings
            </h3>
          </div>

          <h1 className="text-5xl md:text-[3.6rem] xl:text-[4rem] font-bold leading-[1.15] text-white my-6 drop-shadow-md tracking-tight">
            Delicious Foods With <br className="hidden md:block" />
            Wonderful Eating
          </h1>

          <p className="text-slate-200 text-base md:text-[17px] max-w-lg leading-relaxed font-normal shadow-sm">
          ...
          <div className="relative mt-10 w-full max-w-[500px]">
            ...
            <div className="flex bg-white rounded-full p-[6px] pl-6 relative items-center border-[8px] border-white/10 bg-clip-padding backdrop-blur-sm shadow-[0_15px_30px_rgba(0,0,0,0.3)]">
              <input
                placeholder="Search . . ."
                className="flex-1 outline-none text-slate-800 bg-transparent placeholder:text-slate-400 text-[15px] font-medium w-full"
              />
              <button className="bg-[#fb923c] hover:bg-orange-600 text-white px-8 md:px-[2.5rem] py-3.5 md:py-[15px] rounded-full font-bold transition-colors duration-300 text-[15px] shadow-sm">
                Search
              </button>
            </div>

            {/* Tiny abstract decoration left of search box */}
            <div className="absolute bottom-6 -left-12 text-orange-500 opacity-90 animate-pulse">
            ...
            {/* Tiny tomato icon */}
            <div className="absolute -bottom-[22px] left-[55%] transform -translate-x-1/2 drop-shadow-lg text-[22px] pointer-events-none hover:-translate-y-2 transition-transform duration-300 cursor-pointer">
              🍅
            </div>
          </div>
        </div>
        ...
        <div className="relative flex justify-center lg:justify-end items-center h-full animate-in zoom-in duration-700 delay-150 relative z-10 w-full pr-0 lg:pr-8 xl:pr-12">
          ...
          <div className="relative isolate flex items-center justify-center rounded-full p-[12px] w-[320px] h-[320px] sm:w-[420px] sm:h-[420px] lg:w-[500px] lg:h-[500px] xl:w-[540px] xl:h-[540px] group shadow-[0_30px_60px_rgba(0,0,0,0.5)] bg-gradient-to-b from-white via-white/80 to-[#f97316]">
          ...
          {/* 35% Off Badge */}
          <div className="absolute xl:top-2 -left-4 sm:top-4 sm:left-4 z-30 bg-[#f97316] text-white w-24 h-24 lg:w-[130px] lg:h-[130px] rounded-full flex flex-col items-center justify-center shadow-2xl border-[5px] lg:border-[8px] border-white transform hover:scale-110 rotate-[-5deg] transition-transform duration-300">
            <span className="text-[28px] lg:text-[40px] font-extrabold leading-none tracking-tight drop-shadow-sm">35%</span>
            <span className="text-lg lg:text-[28px] font-pacifico leading-tight text-white drop-shadow-sm mt-1">off</span>
          </div>

          {/* Decorative floating elements nearby */}
          <div className="absolute top-10 lg:top-16 -left-6 lg:-left-10 text-orange-500 z-0 animate-spin-slow" style={{ animationDuration: '20s' }}>
          ...
          <div className="absolute top-0 right-10 lg:right-20 text-3xl drop-shadow-xl z-20 animate-pulse">
            🌿
          </div>
        </div>
      </div>

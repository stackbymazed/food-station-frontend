                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {meal.options.filter(o => o.type === 'size').map((opt) => (
                            <button
                                ...
                                className={`flex items-center p-5 rounded-3xl border-2 transition-all duration-300 group cursor-pointer ${selectedSize === opt.name
                                    ? "border-orange-600 bg-orange-50/50"
                                    : "border-slate-100 hover:border-slate-300 bg-white"
                                    }`}
                            >
                                <div className={`w-6 h-6 rounded-full border-2 mr-4 flex items-center justify-center transition-all ${selectedSize === opt.name
                                    ? "border-orange-600"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {meal.options.filter(o => o.type === 'addon').map((opt) => (
                            <button
                                ...
                                className={`flex items-center p-4 rounded-3xl border transition-all duration-300 group cursor-pointer ${selectedAddons.includes(opt.id)
                                    ? "bg-slate-900 border-slate-900 shadow-xl shadow-slate-200"
                                    : "bg-white border-slate-100 hover:border-slate-300"
                                    }`}
                            >
                                <div className={`w-6 h-6 rounded-xl border flex items-center justify-center transition-all ${selectedAddons.includes(opt.id)
            ...
            <div className="pt-8 border-t border-slate-100">
                ...
                <div className="flex flex-col gap-4 mt-8">
                    <button
                        ...
                        className="group relative overflow-hidden bg-orange-600 hover:bg-orange-500 text-white font-black py-6 rounded-2xl sm:rounded-[2rem] shadow-xl shadow-orange-100 transition-all duration-500 active:scale-95 text-sm uppercase tracking-[0.15em] flex items-center justify-center gap-3 w-full"
                    >
                        {/* Shine animation effect */}
                        <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-in-out" />

                        <ShoppingCart size={20} className="group-hover:-translate-y-1 group-hover:translate-x-1 transition-transform duration-300" />
                        <span className="whitespace-nowrap">Add To Cart</span>
                    </button>

                    <button
                        onClick={handlePayToProceed}
                        className="group relative overflow-hidden bg-slate-900 hover:bg-slate-800 text-white font-black py-6 rounded-2xl sm:rounded-[2rem] shadow-xl shadow-slate-200 transition-all duration-500 active:scale-95 text-sm uppercase tracking-[0.15em] flex items-center justify-center gap-3 border-2 border-slate-900 w-full"
                    >
                        <Wallet size={20} className="text-orange-500 group-hover:scale-110 transition-transform duration-300" />
                        <span className="whitespace-nowrap">Pay to Proceed</span>
                    </button>
                </div>
            </div>
        </div>
